...
from starbot.core import app_commands
...
def test_dpy_app_commands_reexports():
    ...
    if missing_attrs:
        pytest.fail(
            "starbot.core.app_commands is missing these names from discord.app_commands: "
            + ", ".join(missing_attrs)
        )
...
def test_dpy_app_commands_checks_reexports():
    ...
    if missing_attrs:
        pytest.fail(
            "starbot.core.app_commands.checks is missing these names from discord.app_commands: "
            + ", ".join(missing_attrs)
        )
