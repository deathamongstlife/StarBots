...
from starbot.pytest.downloader import *

from starbot.cogs.downloader.repo_manager import Installable
from starbot.cogs.downloader.repo_manager import Candidate, ProcessFormatter, RepoManager, Repo
from starbot.cogs.downloader.errors import (
    AmbiguousRevision,
    ExistingGitRepo,
    GitException,
    UnknownRevision,
...
async def test_add_repo(monkeypatch, repo_manager):
    monkeypatch.setattr("starbot.cogs.downloader.repo_manager.Repo._run", fake_run_noprint)
    monkeypatch.setattr(
        "starbot.cogs.downloader.repo_manager.Repo.current_commit", fake_current_commit
    )
...
async def test_lib_install_requirements(monkeypatch, library_installable, repo, tmpdir):
    monkeypatch.setattr("starbot.cogs.downloader.repo_manager.Repo._run", fake_run_noprint)
    monkeypatch.setattr(
        "starbot.cogs.downloader.repo_manager.Repo.available_libraries", (library_installable,)
    )
