from Star-Utils import Cog, CogsUtils, Menu, Settings  # isort:skip
...
class CodeSnippets(DashboardIntegration, Cog):
    ...
    async def fetch_github_pr_diff_snippet(
        ...
        source, ret, __, code = self._snippet_to_codeblock(
            source="GitHub",
            file_contents=file_contents,
            file_path=f"Diff Pull Request {pr_number} in {repo}",
            start_line=start_line,
            end_line=end_line,
        )
    ...
    async def fetch_github_commit_diff_snippet(
        ...
        source, ret, __, code = self._snippet_to_codeblock(
            source="GitHub",
            file_contents=file_contents,
            file_path=f"Diff Commit `{commit_hash}` in {repo}",
            start_line=start_line,
            end_line=end_line,
        )
        return source, ret, "diff", code
