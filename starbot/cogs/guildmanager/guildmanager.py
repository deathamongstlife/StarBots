...
from starbot.core.utils.chat_formatting import bold, humanize_list, humanize_number, inline
...
class GuildManager(Cog):
    ...
    def __init__(self, bot: Red) ->None:
        ...
        self.logs = CogsUtils.get_logger("GuildManager")
    ...
    async def blacklist_remove(self, ctx: commands.Context, *guild_ids: int):
        ...
        async with self.config.blacklist() as blacklist:
            success = [n for n in guild_ids if n in blacklist]
            for guild_id in success:
                blacklist.remove(guild_id)
        failed = [n for n in guild_ids if n not in success]
        content = ''
        if success:
            content += _('The following guilds have been removed from blacklist: {}').format(
                humanize_list([inline(str(n)) for n in sorted(success)])
            )
        if failed:
            content += _('\nThe following guilds were not in the blacklist: {}').format(
