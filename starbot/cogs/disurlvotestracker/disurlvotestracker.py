from Star-Utils import Cog, Loop, Menu, Settings  # isort:skip
from Star-Utils.settings import CustomMessageConverter  # isort:skip
...
class DisurlVotesTracker(DashboardIntegration, Cog):
    ...
    async def on_webhook_receive(self, payload: typing.Dict[str, typing.Any]) -> None:
        ...
        start_month = datetime.datetime.now(tz=datetime.timezone.utc).replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        number_member_monthly_votes = len(
            [vote for vote in member_data["votes"] if datetime.datetime.now(tz=datetime.timezone.utc) >= start_month]
        ) + 1
