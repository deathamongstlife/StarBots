from Star-Utils import Cog, Loop, CogsUtils, Menu, Settings  # isort:skip
...
from starbot.core.utils.chat_formatting import humanize_list, inline
...
class GetDocs(Cog):
    ...
    def __init__(self, bot: Red) -> None:
        ...
        self.config.register_global(
            ...
            caching=True,
            enabled_sources=["discord.py", "starbot", "python", "aiohttp", "discordapi"],
        )
    ...
    @app_commands.describe(
        ...
        query="Your search. (`events` to get all dpy events, for `discord.py`, `starbot` and `pylav` source only)",
    )
    async def rtfm(
        self,
        ctx: commands.Context,
    ...
    async def rtfm_query_autocomplete(
        ...
        if not current and source.name in ("discord.py", "starbot", "pylav"):
    ...
    async def on_assistant_cog_add(
        self, assistant_cog: typing.Optional[Cog] = None
...
class Source:
    ...
    async def search(
            ...
            if self.name == "discord.py":
                ...
                for name in dir(aiohttp.ClientSession):
                    ...
                    if query.lower() == name:
                        ...
                        break
        if self.name in ("discord.py", "starbot"):
        ...
        if self.name in ("discord.py", "starbot", "pylav") and query == "events":
            exclude_std = True
            if self.name != "pylav":
                matches = [
                    item
