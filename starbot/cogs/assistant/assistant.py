...
class Assistant(
    ...
    def __init__(self, bot: Red, *args, **kwargs):
        ...
        self.db: DB = DB()
        self.mp_pool = Pool()
        self.logs = CogsUtils.get_logger("Assistant")
    ...
    async def register_function(
        ...
        cog = self.bot.get_cog(cog_name)
        if not cog:
            log.info(fail("commands.Cog is not loaded or does not exist"))
            return False
        ...
        if not hasattr(cog, function_name):
            log.info(fail(f"commands.Cog does not have a function called {function_name}"))
            return False
