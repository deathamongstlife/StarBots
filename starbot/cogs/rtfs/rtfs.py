...
from starbot.core.dev_commands import cleanup_code
...
if TYPE_CHECKING:
    from starbot.cogs.downloader import Downloader
    from starbot.core.dev_commands import Dev
...
async def format_and_send(ctx: commands.Context, obj: Any, *, is_owner: bool = False) -> None:
    ...
    if full_module:
        ...
        elif full_module.startswith("starbot."):
            is_installed = not starbot.version_info.dirty
            if starbot.version_info.dev_release:
                red_commit = starbot.version_info.short_commit_hash or "V3/develop"
            else:
...
class RTFS(Cog):
    def __init__(self, bot: Red):
        super().__init__(bot)
        self.logs = CogsUtils.get_logger("RTFS")
        self.logs = CogsUtils.get_logger("RTFS")
        self.logs = CogsUtils.get_logger("RTFS")
        self.bot = bot
