...
class GuildSelect(Select["GuildSelectView"]):
    def __init__(self, ctx: commands.Context, guilds: list[discord.Guild]):
        super().__init__(
            placeholder="Select guilds to sync role to", min_values=1, max_values=2
        )
        self.ctx = ctx
        self.guilds = guilds
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.view.chosen_guilds = [
            self.ctx.bot.get_guild(int(val)) for val in self.values
        ]
        self.view.stop()
...
class GuildSelectView(View):
    def __init__(self, ctx: commands.Context):
        ...
        self.add_item(GuildSelect(ctx, self.guilds))
