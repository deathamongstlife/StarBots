from Star-Utils import Cog, CogsUtils, Menu  # isort:skip
...
class KoFiTracker(Cog):
    ...
    async def get_kofi_page_details(self, kofi_page_url: str) -> typing.Dict[str, str]:
        ...
        data = {
            ...
            "avatar_url": soup.select_one("img#profilePicture")["src"] if soup.select_one("img#profilePicture") is not None else "https://ko-fi.com/img/anon2.png",
            ...
            "goal_title": None if soup.select_one(".kfds-btm-mrgn-24 > .text-left").text.startswith("\n") and soup.select_one(".kfds-btm-mrgn-24 > .text-left").text.endswith("\n") else soup.select_one(".kfds-btm-mrgn-24 > .text-left").text,
            "goal_description": getattr(soup.select_one(".goal-description"), "text", None),
            "current_percentage": soup.select_one(".text-left > .kfds-font-bold").text if soup.select_one(".text-left > .kfds-font-bold").text != "Report Abuse" else None,
    ...
    async def on_webhook_receive(self, payload: typing.Dict[str, typing.Any]) -> None:
        if payload.get("type") not in ("Donation", "Subscription", "Shop Order") or "verification_token" not in payload:
            return
        for channel_id, channels_data in (await self.config.all_channels()).items():
            for kofi_page_url, data in channels_data["kofi_pages"].items():
                ...
                embed.add_field(name=_("Amount:"), value=f"{payload['amount']} {payload['currency']}", inline=False)
                if payload["message"]:
                ...
                if payload["type"] == "Subscription":
                    ...
                    if payload["is_first_subscription_payment"]:
                        embed.add_field(name="\u200b", value=_("(First Subscription Payment)"), inline=False)
                elif payload["type"] == "Shop Order":
                    embed.add_field(name=_("Shop Items:"), value="\n".join([f"**•** **{item['quantity']}x** {item['variation_name']}" for item in payload["shop_items"]]), inline=True)
                ...
                view.add_item(
                    discord.ui.Button(
                        emoji="❤️",
                        label=_("Support {display_name} on KoFi!").format(display_name=kofi_page_details["display_name"]),
                ...
                try:
                    ...
                    self.logger.error(f"Error when sending KoFi notification in `{channel.name}` ({channel.id}).", exc_info=e)
    ...
    async def kofiprofile(self, ctx: commands.Context, kofi_page_url: str) -> None:
        ...
        embed.add_field(name=_("Received Amount:"), value=kofi_page_details["received_amount"], inline=False)
        if kofi_page_details["goal_title"] is not None:
            embed.add_field(name=_("Goal Title:"), value=kofi_page_details["goal_title"], inline=False)
            embed.add_field(name=_("Goal Description:"), value=kofi_page_details["goal_description"], inline=False)
            embed.add_field(name=_("Current Percentage:"), value=kofi_page_details["current_percentage"], inline=False)
            embed.add_field(name=_("Of Goal Total:"), value=kofi_page_details["of_goal_total"], inline=False)
        embed.set_footer(text=_("Fetched from KoFi."), icon_url="https://storage.ko-fi.com/cdn/nav-logo-stroke.png")
    ...
    async def add(
        ...
        types: commands.Greedy[typing.Literal["Donation", "Subscription", "Shop Order"]] = ["Donation", "Subscription", "Shop Order"],
    ...
    @setkofitracker.command(aliases=["-"])
    async def remove(self, ctx: commands.Context, channel: typing.Optional[typing.Union[discord.TextChannel, discord.VoiceChannel, discord.Thread]], kofi_page_url: str) -> None:
    ...
    async def list(self, ctx: commands.Context, channel: typing.Optional[typing.Union[discord.TextChannel, discord.VoiceChannel, discord.Thread]]) -> None:
