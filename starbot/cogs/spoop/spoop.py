...
BaseCog = getattr(commands, "commands.commands.commands.commands.commands.Cog", object)


class Spoop(Cog, ErisEventMixin):
    def __init__(self, bot):
        super().__init__(bot)
        self.logs = CogsUtils.get_logger("Spoop")
        self.bot = bot
    ...
    async def randomly_spoop(self, message: discord.Message):
        randomly_allowed: bool = random.random() <= 0.001
