...
def get_optiontype_description(optiontype: discord.AppCommandOptionType) -> str:
    return {
        discord.AppCommandOptionType.string: "Accepts any user inputted text as an argument.",
        discord.AppCommandOptionType.integer: "Only allows integer input for the argument.",
        discord.AppCommandOptionType.boolean: "Allows either `True` or `False` as input.",
        discord.AppCommandOptionType.user: "Refers to a member of the server, accepting username or IDs as input.",
...
class OptionPickerSelect(discord.ui.Select):
    ...
    def create_options() -> List[discord.SelectOption]:
        ...
        for name, member in discord.AppCommandOptionType.__members__.items():
            ...
            option = discord.SelectOption(
                label=name.title(),
                value=member.value,
                description=get_optiontype_description(member),
    ...
    async def callback(self, interaction: discord.Interaction):
        value = discord.AppCommandOptionType(int(self.values[0])) if self.values[0] != "-1" else -1
...
class OptionPickerView(BaseView):
    def __init__(self, *, timeout: int):
        ...
        self.value: Optional[discord.AppCommandOptionType] = None
