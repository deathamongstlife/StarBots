...
from starbot.core.utils._internal_utils import cli_level_to_log_level
...
async def interactive_config(red, token_set, prefix_set, *, print_header=True):
    ...
    if print_header:
        print("starbot | Configuration process\n")
...
def parse_cli_flags(args):
    parser = argparse.ArgumentParser(
        description="starbot", usage="starbot <instance_name> [arguments]"
    )
    parser.add_argument(
        "--version", "-V", action="store_true", help="Show starbot's current version"
    )
    parser.add_argument("--debuginfo", action="store_true", help="Show debug information.")
    parser.add_argument(
        "--list-instances",
        action="store_true",
        help="List all instance names setup with 'starbot-setup'",
    )
    parser.add_argument(
        "--edit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "starbot should be owner, this has "
    ...
    parser.add_argument(
        ...
        "to the system that is hosting starbot should be  "
    ...
    parser.add_argument(
        "--no-cogs", action="store_true", help="Starts starbot with no cogs loaded, only core"
    )
    parser.add_argument(
        "--load-cogs",
        type=str,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Makes starbot quit with code 0 just before the "
    ...
    parser.add_argument("--token", type=str, help="Run starbot with the given token.")
    parser.add_argument(
        "--no-instance",
        action="store_true",
        help=(
            "Run starbot without any existing instance. "
            "The data will be saved under a temporary folder "
            "and deleted on next system restart."
        ),
    )
    parser.add_argument(
        "instance_name", nargs="?", help="Name of the bot instance created during `starbot-setup`."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        " as starbot is not prepared to work without all intents.\n"
