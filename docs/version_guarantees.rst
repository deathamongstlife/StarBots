...
the ``starbot`` module (excluding its submodules), ``starbot.core`` package,
...
Any other name (function, class, attribute) in the ``starbot`` package is considered private,
even if it doesn't start with "_".
Lack of ``__all__`` in the module means that all of its names are considered private APIs.

Anything in the ``starbot.cogs`` and ``starbot.vendored`` modules or any of their submodules is specifically
